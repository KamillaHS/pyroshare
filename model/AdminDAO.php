...
//        $query->execute();
        ...
        $handle = $dbCon->prepare("UPDATE `backendstyle` SET `isUsed` = 1 WHERE StyleID = ?");
        $handle->bindParam(1, $themeID);
        $handle->execute();

        $handle = $dbCon->prepare("UPDATE `backendstyle` SET `isUsed` = 0 WHERE StyleID != ?");
        $handle->bindParam(1, $themeID);
        $handle->execute();
